 ...
 * COMP 3021
 * 
 * For New JavaJDK over verson 11(even using compliance level 1.8)
 * project>properties>java build path>libraries>add Externals JARs>add downloaded javafx lib files>apply
 * run>run configurations>Arguments>VMarguments>
 * --module-path C:\Users\User\javafx\javafx-sdk-17.0.1\lib --add-modules=javafx.controls,javafx.fxml
 * 
 * @author valerio
 *
 */
public class NoteBookWindow extends Application {
	...
	private VBox addVBox() {
		...
		hbox1.getChildren().add(buttonAddFolder);
		...
		buttonDeleteNote.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {			
				...
				}else
				{
					for (Folder folder: noteBook.getFolders()) {
						if (folder.getName().equals(currentFolder)) {
							 removed = folder.removeNote(currentNote);
